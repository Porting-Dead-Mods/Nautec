...
public class AugmentationViewerScreen extends Screen {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (AugmentSlot slot : fullSlots) {
            displayAugment(guiGraphics, slot, augments.get(slot), 150, y += 20);
        }
    ...
    public void displayAugment(GuiGraphics graphics, AugmentSlot slot, Augment aug, int x, int y) {
        graphics.drawString(this.font, slot.getName() + ":", x, y, 0xFFFFFFFF);
        graphics.drawString(this.font, aug == null ? "    No Augment in slot" : "    " + aug.getAugmentType().toString(), x, y + 10, 0xFFFFFFFF);
    }
    ...
    public void displayAugment(GuiGraphics graphics, Supplier<AugmentSlot> augmSupplier, Map<AugmentSlot, Augment> augments, int x, int y) {
        graphics.drawString(this.font, augmSupplier.get().getName() + ":", x, y, 0xFFFFFFFF);
        graphics.drawString(this.font, augments.get(augmSupplier.get()) == null ? "    No Augment in slot" : "    " + augments.get(augmSupplier.get()).getAugmentType().toString(), x, y + 10, 0xFFFFFFFF);
    }
    ...
    public void displayAugmentB(GuiGraphics graphics, Supplier<AugmentSlot> augmSupplier, Map<AugmentSlot, Augment> augments, int x, int y) {
        graphics.drawString(this.font, augmSupplier.get().getName() + ": " + ((augments.get(augmSupplier.get()) == null) ? "None" : augments.get(augmSupplier.get()).getAugmentType().toString()), x, y, 0xFFFFFFFF);
        // graphics.drawString(this.font, augments.get(augmSupplier.get())==null?"    No Augment in slot":"    "+augments.get(augmSupplier.get()).getAugmentType().toString(), x, y+10, 0xFFFFFFFF);
    }
