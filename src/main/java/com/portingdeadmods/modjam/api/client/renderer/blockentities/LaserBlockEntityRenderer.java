...
public abstract class LaserBlockEntityRenderer<T extends LaserBlockEntity> implements BlockEntityRenderer<T> {
    ...
    public void render(T blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        for (Direction direction : blockEntity.getLaserOutputs()) {
            ...
            renderBeam(blockEntity, originPos, targetPos, direction, poseStack, bufferSource, partialTick);
            poseStack.pushPose();
            {
                poseStack.scale(0.125f, 1, 0.125f);
                poseStack.translate(3.375f, 0, 3.375f);
                renderBeaconBeam(poseStack, bufferSource, partialTick, blockEntity.getLevel().getGameTime(),
                        0, laserDistance + 1, FastColor.ARGB32.color(87, 125, 100));
            }
            poseStack.popPose();
        }
    }

    private void renderBeam(T blockEntity, BlockPos originPos, BlockPos targetPos, Direction direction, PoseStack poseStack, MultiBufferSource bufferSource, float partialTicks) {
        float f = blockEntity.getLaserScale(partialTicks);
        float f1 = blockEntity.getLaserAnimTime() + partialTicks;
